...
export default class InstanceRow extends Component {
  constructor(props) {
    super(props);
  }

  formatPrice = (price, suffix) => (price === 0 ? null : `$${Math.round(price)} ${suffix}`);
  ...
  render() {
    ...
    const companyRelation = `[${RelationshipString}]`;
            ...
            <Row>
              <Text style={myStyle.text}>
                {this.formatPrice(
                  ...
                  CurrentHirePeriodString.toLowerCase()
                )}
              </Text>
            </Row>
            <Row>
              <Text style={myStyle.text}>
                {this.formatPrice(object[`Nhr${hireType}`], "hourly")}
              </Text>
            </Row>
          </Col>
